...
def httpspoof(args, validate_time, send, client, ansi_clear, broadcast, data):
    if len(args) == 3:
        ...
        xxxx = '''%s============= (%sTARGET%s) ==============
            %s URL:%s %s
            %sTIME:%s %s
          %sMETHOD:%s %s'''%(Fore.LIGHTBLACK_EX, Fore.GREEN, Fore.LIGHTBLACK_EX, Fore.MAGENTA, Fore.LIGHTWHITE_EX, url, Fore.MAGENTA, Fore.LIGHTWHITE_EX, secs, Fore.MAGENTA, Fore.LIGHTWHITE_EX, 'HTTP SPOOF')
